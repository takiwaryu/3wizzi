...
if(get_option($tpl->name . '_shortcodes3_state', 'Y') == 'Y') {  
	 ...
	 *
	 **/
	if(!function_exists('gavern_tss_social')) {
		// Usage:
		// [social fb="FB_URL" twitter="TWITTER_URL" gplus="GPLUS_URL" rss="RSS_URL"]
		function gavern_tss_social($atts) {   
			// extract the attributes
			extract(shortcode_atts(array("fb" => "", "twitter" => "", "gplus" => "", "rss" => ""), $atts));
			
			$output = '<div class="social-icons">';
			
			if(trim($fb) != '') {
				$output .= '<a class="gk-fb" href="'.$fb.'">'.__('Facebook', GKTPLNAME).'</a>';	
			}
			
			if(trim($twitter) != '') { 
				$output .= '<a class="gk-twitter" href="'.$twitter.'">'.__('Twitter', GKTPLNAME).'</a>';
			} 
			
			if(trim($gplus) != '') {
				$output .= '<a class="gk-gplus" href="'.$gplus.'">'.__('Google+', GKTPLNAME).'</a>'; 
			}
			
			if(trim($rss) != '') {
				$output .= '<a class="gk-rss" href="'.$rss.'">'.__('RSS', GKTPLNAME).'</a>';
			}
		...
		// add the shortcode to system
		add_shortcode('social', 'gavern_tss_social');
	}
	 ...
	 *
	 **/
	if(!function_exists('gavern_tss_toplist')) {
		// Usage:
		// [toplist]
		// item1
		// item2
		// item3
		// [/toplist]
		function gavern_tss_toplist($atts, $content) {   
			// generate output
			$output = "<ol class=\"gk-toplist\">\n";
			// get the lines
			$lines = preg_split( '/\r\n|\r|\n/', $content );
			// generate the list items
			if(count($lines)) {
		...
		// add the shortcode to system
		add_shortcode('toplist', 'gavern_tss_toplist');
	}
}  
