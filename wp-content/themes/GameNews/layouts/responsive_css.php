...
<style type="text/css">
	.gk-page, .gk-page-wrap { max-width: <?php echo get_option($tpl->name . '_template_width', 980); ?>px; }
	<?php if(
		get_option($tpl->name . '_page_layout', 'right') != 'none' && 
		gk_is_active_sidebar('sidebar') && 
		($fullwidth != true)
	) : ?>
	#gk-mainbody-columns > aside { width: <?php echo (is_home()) ? get_option($tpl->name . '_sidebar_width_homepage', '36') : get_option($tpl->name . '_sidebar_width', '30'); ?>%;}
	#gk-mainbody-columns > section { width: <?php echo 100 - ((is_home()) ? get_option($tpl->name . '_sidebar_width_homepage', '36') : get_option($tpl->name . '_sidebar_width', '30')); ?>%; }
	<?php else : ?>
...
// check the dependencies for the tablet.css file
